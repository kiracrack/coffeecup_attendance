...
Public Class frmMainMenu
    ...
    Public Sub CheckTime()
        If Now.ToShortTimeString > CDate("00:00:00") And Now.ToShortTimeString < CDate("12:30:00") Then
            radMorning.Checked = True
        Else
            radAfterNoon.Checked = True
        End If
    ...
    'Public Sub OfflineModeLogin()
    '    If countqrylocal("tblemployees", "employeeid='" & txtEmployeeID.Text & "' and resigned=0") = 0 Then
    ...
    '        Exit Sub
    '    End If
    '    If qrysingledatalocal("attendancepassword", "attendancepassword", "tblemployees where employeeid='" & txtEmployeeID.Text & "' and resigned=0") = "" Then
    '        MessageBox.Show("Employee has found that attendance password currently not save. Please connect online mode to save new password!", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
    '        Exit Sub
    '    Else
    '        Dim officeid As String = qrysingledatalocal("officeid", "officeid", "tblemployees where employeeid='" & txtEmployeeID.Text & "'")
    '        Dim onetimeinout As Boolean = CBool(qrysingledatalocal("onetimeinout", "onetimeinout", "tblemployees where employeeid='" & txtEmployeeID.Text & "'"))
    '        Dim getPassword As String = EncryptTripleDES(UCase(txtEmployeeID.Text) + txtPassword.Text)
    '        If countqrylocal("tblemployees", "employeeid='" & txtEmployeeID.Text & "' and attendancepassword='" & getPassword & "'") > 0 Then
    '            Dim messagestatus As String = ""
    '            If radMorning.Checked = True Then
    '                If countqrylocal("tblpayrollattendancelog", "logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = 0 Then
    '                    local_com.CommandText = "insert into tblpayrollattendancelog (employeeid,officeid,logindate,1st_timein) values('" & txtEmployeeID.Text & "','" & officeid & "','" & txtServerdate.Text & "','" & ConvertServerTime(txtTime.Text) & "')" : local_com.ExecuteNonQuery()
    '                    messagestatus = "login your morning session"
    '                    local_com.CommandText = "insert into tblattendanceimage (employeeid,officeid,logindate) values('" & txtEmployeeID.Text & "','" & officeid & "','" & txtServerdate.Text & "')" : local_com.ExecuteNonQuery()
    '                    If ActiveCamera = True Then
    '                        UpdateImagelocal("employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'", "1st_timein", "tblattendanceimage", CLearDateString(Now.ToString), TakeScreenShot(PicturePanel), Me)
    '                    End If
    '                Else
    '                    If qrysingledatalocal("1st_timein", "1st_timein", "tblpayrollattendancelog where logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = "" Then
    '                        local_com.CommandText = "update tblpayrollattendancelog set 1st_timein='" & ConvertServerTime(txtTime.Text) & "' where employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'" : local_com.ExecuteNonQuery()
    '                        If ActiveCamera = True Then
    '                            UpdateImagelocal("employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'", "1st_timein", "tblattendanceimage", CLearDateString(Now.ToString), TakeScreenShot(PicturePanel), Me)
    '                        End If
    '                        messagestatus = "login your morning session"
    '                    Else
    '                        If qrysingledatalocal("1st_timeout", "1st_timeout", "tblpayrollattendancelog where logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = "" Then
    '                            local_com.CommandText = "update tblpayrollattendancelog set 1st_timeout='" & ConvertServerTime(txtTime.Text) & "' where employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'" : local_com.ExecuteNonQuery()
    '                            If ActiveCamera = True Then
    '                                UpdateImagelocal("employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'", "1st_timeout", "tblattendanceimage", CLearDateString(Now.ToString), TakeScreenShot(PicturePanel), Me)
    '                            End If
    '                            messagestatus = "logout your morning session"
    '                        Else
    '                            MessageBox.Show("You have already logout morning session!", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error)
    '                            Exit Sub
    '                        End If
    '                    End If
    '                End If
    '            ElseIf radAfterNoon.Checked = True Then
    '                If countqrylocal("tblpayrollattendancelog", "[1st_timein] is not null and [1st_timeout] is null and logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") > 0 And onetimeinout = False Then
    '                    MessageBox.Show("You have not yet logout your morning session!", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error)
    '                    Exit Sub
    '                End If
    '                If countqrylocal("tblpayrollattendancelog", "logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = 0 Then
    '                    local_com.CommandText = "insert into tblpayrollattendancelog (employeeid,officeid,logindate,2nd_timein) values('" & txtEmployeeID.Text & "','" & officeid & "','" & txtServerdate.Text & "','" & ConvertServerTime(txtTime.Text) & "')" : local_com.ExecuteNonQuery()
    '                    local_com.CommandText = "insert into tblattendanceimage  (employeeid,officeid,logindate) values('" & txtEmployeeID.Text & "','" & officeid & "','" & txtServerdate.Text & "')" : local_com.ExecuteNonQuery()
    '                    If ActiveCamera = True Then
    '                        UpdateImagelocal("employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'", "2nd_timein", "tblattendanceimage", CLearDateString(Now.ToString), TakeScreenShot(PicturePanel), Me)
    '                    End If
    '                    messagestatus = "login your afternoon session"
    '                Else
    '                    If qrysingledatalocal("2nd_timein", "2nd_timein", "tblpayrollattendancelog where logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = "" And onetimeinout = False Then
    '                        local_com.CommandText = "update tblpayrollattendancelog set 2nd_timein='" & ConvertServerTime(txtTime.Text) & "' where employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'" : local_com.ExecuteNonQuery()
    '                        If ActiveCamera = True Then
    '                            UpdateImagelocal("employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'", "2nd_timein", "tblattendanceimage", CLearDateString(Now.ToString), TakeScreenShot(PicturePanel), Me)
    '                        End If
    '                        messagestatus = "login your afternoon session"
    '                    Else
    '                        If qrysingledatalocal("2nd_timeout", "2nd_timeout", "tblpayrollattendancelog where logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = "" Then
    '                            local_com.CommandText = "update tblpayrollattendancelog set 2nd_timeout='" & ConvertServerTime(txtTime.Text) & "' where employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'" : local_com.ExecuteNonQuery()
    '                            If ActiveCamera = True Then
    '                                UpdateImagelocal("employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'", "2nd_timeout", "tblattendanceimage", CLearDateString(Now.ToString), TakeScreenShot(PicturePanel), Me)
    '                            End If
    '                            messagestatus = "logout your afternoon session"
    '                        Else
    '                            MessageBox.Show("You have already logout your afternoon session!", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error)
    '                            Exit Sub
    '                        End If
    '                    End If
    '                End If
    '            ElseIf radovertime.Checked = True Then
    '                If countqrylocal("tblpayrollattendancelog", "[2nd_timein]<>'' and [2nd_timeout]='' and logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") > 0 Then
    '                    MessageBox.Show("You have not yet logout your afternoon session!", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error)
    '                    Exit Sub
    '                End If
    '                If countqrylocal("tblpayrollattendancelog", "logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = 0 Then
    '                    local_com.CommandText = "insert into tblpayrollattendancelog (employeeid,officeid,logindate,3rd_timein) values('" & txtEmployeeID.Text & "','" & officeid & "','" & txtServerdate.Text & "','" & ConvertServerTime(txtTime.Text) & "')" : local_com.ExecuteNonQuery()
    '                    local_com.CommandText = "insert into tblattendanceimage (employeeid,officeid,logindate) values ('" & txtEmployeeID.Text & "','" & officeid & "','" & txtServerdate.Text & "')" : local_com.ExecuteNonQuery()
    '                    If ActiveCamera = True Then
    '                        UpdateImagelocal("employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'", "3rd_timein", "tblattendanceimage", CLearDateString(Now.ToString), TakeScreenShot(PicturePanel), Me)
    '                    End If
    '                    messagestatus = "login your overtime session"
    '                Else
    '                    If qrysingledatalocal("3rd_timein", "3rd_timein", "tblpayrollattendancelog where logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = "" Then
    '                        local_com.CommandText = "update tblpayrollattendancelog set 3rd_timein='" & ConvertServerTime(txtTime.Text) & "' where employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'" : local_com.ExecuteNonQuery()
    '                        If ActiveCamera = True Then
    '                            UpdateImagelocal("employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'", "3rd_timein", "tblattendanceimage", CLearDateString(Now.ToString), TakeScreenShot(PicturePanel), Me)
    '                        End If
    '                        messagestatus = "login your overtime session"
    '                    Else
    '                        If qrysingledatalocal("3rd_timeout", "3rd_timeout", "tblpayrollattendancelog where logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = "" Then
    '                            local_com.CommandText = "update tblpayrollattendancelog set 3rd_timeout='" & ConvertServerTime(txtTime.Text) & "' where employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'" : local_com.ExecuteNonQuery()
    '                            If ActiveCamera = True Then
    '                                UpdateImagelocal("employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'", "3rd_timeout", "tblattendanceimage", CLearDateString(Now.ToString), TakeScreenShot(PicturePanel), Me)
    '                            End If
    '                            messagestatus = "logout your overtime session"
    '                        Else
    ...
    '                End If
    '            End If
    '            txtEmployeeID.Text = "EMPLOYEE ID" : txtPassword.Text = "Password" : txtEmployeeID.Focus() : txtEmployeeID.SelectAll()
    '            MessageBox.Show("Your are successfully " & messagestatus & "!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information)
    '        Else
    ...
    Public Sub OnlineModeLogin()
        If CBool(qrysingledata("enabletimeconsoleipblocking", "enabletimeconsoleipblocking", "tblgeneralsettings")) = True And countqry("tbltimeconsoleaddress", "ipaddress='" & GetLocalIPAddress() & "'") = 0 Then
            MessageBox.Show("Your time console is not allowed to access coffeecup server!", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End
        End If
        ...
        Dim getOfficeDept As String = qrysingledata("depid", "depid", "tblemployees where employeeid='" & txtEmployeeID.Text & "'")
        If getOfficeDept = "" Then
            MessageBox.Show("Your account has not been assign to department! Please contact human resource management", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Exit Sub
        End If
        If countqry("tbltimeconsoleaddress", "officeid='" & getOfficeDept & "'") > 0 Then
            If countqry("tbltimeconsoleaddress", "ipaddress='" & GetLocalIPAddress() & "' and officeid='" & getOfficeDept & "'") = 0 Then
                MessageBox.Show("You are not allowed to timein or timeout to this computer! Please go to your department", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Exit Sub
            End If
        End If
            ...
            If countqry("tblemployees", "employeeid='" & txtEmployeeID.Text & "' and attendancepassword='" & EncryptTripleDES(UCase(txtEmployeeID.Text) + txtPassword.Text) & "'") > 0 Then
                ...
                If radMorning.Checked = True Then
                    If countqry("tblpayrollattendancelog", "logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = 0 Then
                        com.CommandText = "insert into tblpayrollattendancelog set employeeid='" & txtEmployeeID.Text & "', shiftcode='" & shiftcode & "', officeid='" & officeid & "', logindate='" & txtServerdate.Text & "', 1st_timein='" & ConvertServerTime(txtTime.Text) & "'" : com.ExecuteNonQuery()
                        messagestatus = "login in your AM session"
                        session = "1st_timein"
                    ...
                    Else
                        If qrysingledata("1st_timein", "1st_timein", "tblpayrollattendancelog where logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = "" Then
                            com.CommandText = "update tblpayrollattendancelog set 1st_timein='" & ConvertServerTime(txtTime.Text) & "' where employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'" : com.ExecuteNonQuery()
                            If ActiveCamera = True Then
                                UpdateImage("employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'", "1st_timein", "filedir.tblattendanceimage", TakeScreenShot(PicturePanel), Me)
                            End If
                            session = "1st_timein"
                        ...
                        Else
                            If qrysingledata("1st_timeout", "1st_timeout", "tblpayrollattendancelog where logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = "" Then
                                com.CommandText = "update tblpayrollattendancelog set 1st_timeout='" & ConvertServerTime(txtTime.Text) & "' where employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'" : com.ExecuteNonQuery()
                                If ActiveCamera = True Then
                                ...
                                End If
                                session = "1st_timeout"
                                messagestatus = "logout in your AM session"
                            Else
                                MessageBox.Show("You have already logout morning session!", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error)
                                Exit Sub
                            End If
                ...
                ElseIf radAfterNoon.Checked = True Then
                    ...
                    End If
                    If countqry("tblpayrollattendancelog", "logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = 0 Then
                        com.CommandText = "insert into tblpayrollattendancelog set employeeid='" & txtEmployeeID.Text & "',shiftcode='" & shiftcode & "',officeid='" & officeid & "', logindate='" & txtServerdate.Text & "', 2nd_timein='" & ConvertServerTime(txtTime.Text) & "'" : com.ExecuteNonQuery()
                        com.CommandText = "insert into filedir.tblattendanceimage set employeeid='" & txtEmployeeID.Text & "',officeid='" & officeid & "', logindate='" & txtServerdate.Text & "'" : com.ExecuteNonQuery()
                        If ActiveCamera = True Then
                        ...
                        End If
                        session = "2nd_timein"
                        messagestatus = "login in your PM session"
                    Else
                        If qrysingledata("2nd_timein", "2nd_timein", "tblpayrollattendancelog where logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = "" And onetimeinout = False Then
                            com.CommandText = "update tblpayrollattendancelog set 2nd_timein='" & ConvertServerTime(txtTime.Text) & "' where employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'" : com.ExecuteNonQuery()
                            If ActiveCamera = True Then
                                UpdateImage("employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'", "2nd_timein", "filedir.tblattendanceimage", TakeScreenShot(PicturePanel), Me)
                            End If
                            session = "2nd_timein"
                        ...
                        Else
                            If qrysingledata("2nd_timeout", "2nd_timeout", "tblpayrollattendancelog where logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = "" Then
                                com.CommandText = "update tblpayrollattendancelog set 2nd_timeout='" & ConvertServerTime(txtTime.Text) & "' where employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'" : com.ExecuteNonQuery()
                                If ActiveCamera = True Then
                                ...
                                End If
                                session = "2nd_timeout"
                                messagestatus = "logout in your PM session"
                            Else
                                MessageBox.Show("You have already logout your afternoon session!", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error)
                                Exit Sub
                            End If
                ...
                ElseIf radovertime.Checked = True Then
                    ...
                    End If
                    If countqry("tblpayrollattendancelog", "logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = 0 Then
                        com.CommandText = "insert into tblpayrollattendancelog set employeeid='" & txtEmployeeID.Text & "',shiftcode='" & shiftcode & "',officeid='" & officeid & "', logindate='" & txtServerdate.Text & "', 3rd_timein='" & ConvertServerTime(txtTime.Text) & "'" : com.ExecuteNonQuery()
                        com.CommandText = "insert into filedir.tblattendanceimage set employeeid='" & txtEmployeeID.Text & "',officeid='" & officeid & "', logindate='" & txtServerdate.Text & "'" : com.ExecuteNonQuery()
                        If ActiveCamera = True Then
                    ...
                    Else
                        If qrysingledata("3rd_timein", "3rd_timein", "tblpayrollattendancelog where logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = "" Then
                            com.CommandText = "update tblpayrollattendancelog set 3rd_timein='" & ConvertServerTime(txtTime.Text) & "' where employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'" : com.ExecuteNonQuery()
                            If ActiveCamera = True Then
                                UpdateImage("employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'", "3rd_timein", "filedir.tblattendanceimage", TakeScreenShot(PicturePanel), Me)
                            End If
                            session = "3rd_timein"
                        ...
                        Else
                            If qrysingledata("3rd_timeout", "3rd_timeout", "tblpayrollattendancelog where logindate='" & txtServerdate.Text & "' and employeeid='" & txtEmployeeID.Text & "'") = "" Then
                                com.CommandText = "update tblpayrollattendancelog set 3rd_timeout='" & ConvertServerTime(txtTime.Text) & "' where employeeid='" & txtEmployeeID.Text & "' and logindate='" & txtServerdate.Text & "'" : com.ExecuteNonQuery()
                                If ActiveCamera = True Then
    ...
    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        If conn.State = ConnectionState.Open Then
            txtTime.Text = CDate(GetServerTime()).ToString("HH:mm:ss tt")
        End If
    End Sub
...
#End Region
    ...
    Private Sub cmdChangeLogoutDate_Click(sender As Object, e As EventArgs) Handles cmdChangeLogoutDate.Click
        frmChangeLogoutDate.Show(Me)
    End Sub
End Class
